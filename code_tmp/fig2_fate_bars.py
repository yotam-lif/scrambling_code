r"""Variant of fig2 with panel A redrawn as stacked "fate" bars instead of Couce's arrow plot.

Scratch copy of ``code_figs/fig2_scrambling_exper_res.py``: rows 1 (B, C) and 2 are unchanged,
only A differs.  The published arrow plot (Couce et al. Fig. 5E) is replaced by a presentation
of the same message -- beneficial knockouts become neutral or deleterious in the other
background -- as one 100% bar per direction.
...
Run from anywhere:  python code_tmp/fig2_fate_bars.py
Output:             code_tmp/out_tmp/fig2_fate_bars.pdf
"""
...
FATE_COLORS = ("#b3434d", "#d5d5d5", "#326da8")
...
def create_fate_bars(ax, dfe_anc, dfe_evo, labels):
    ...
    directions = ((dfe_anc, dfe_evo, early, late, 1.0), (dfe_evo, dfe_anc, late, early, 0.0))
    height = 0.40
    ax.set_xlim(0, 100)
    ...
    for source, target, src_label, dst_label, y in directions:
        ...
        left = 0.0
        for fraction, color in zip(100 * fractions, FATE_COLORS):
            ...
            label = ax.text(center, y, f"{fraction:.1f}%", ha="center",
                            ...
                            color="#222222" if color == FATE_COLORS[1] else "white")
        ...
        source_name = "ancestor" if src_label == early else "evolved"
        target_name = "evolved" if dst_label == late else "ancestor"
        ax.text(0, y + height / 2 + 0.30,
                f"Beneficial in {source_name}",
                ha="left", va="bottom", fontsize=16, fontweight="medium")
        ax.text(0, y + height / 2 + 0.08,
                f"Measured in {target_name} ({dst_label})",
                ha="left", va="bottom", fontsize=14, color="#555555")
    ...
    handles = [Rectangle((0, 0), 1, 1, facecolor=c, edgecolor="none") for c in FATE_COLORS]
    names = ["Deleterious", "Neutral", "Beneficial"]
    ax.legend(handles, names, loc="upper left", bbox_to_anchor=(0, 0.045), ncol=3,
              frameon=False, fontsize=13, handlelength=1.0, handletextpad=0.4,
...
def main():
    ...
    out_path = os.path.join(OUT_DIR, "fig2_fate_bars.pdf")
